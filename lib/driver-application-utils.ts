 ...
 */
export function validateDriverApplication(data: DriverApplicationData): {
  isValid: boolean;
  errors: string[];
} {
  const errors: string[] = [];
  ...
  const hasNationalId =
    data.national_id_file_recto || data.national_id_file_verso;
  const hasLicense = data.license_file_recto || data.license_file_verso;
  const hasRegistration =
    data.registration_file_recto || data.registration_file_verso;
...
export async function submitDriverApplication(
  ...
  try {
    ...
    console.log("📝 Submitting driver application...", {
      userId,
      documentCount: Object.keys(driverData).filter((key) =>
        key.includes("file")
      ).length,
...
export async function updateDriverStatus(
  ...
  try {
    ...
    const { data: existingProfile, error: checkError } = await supabase
      .from("profiles")
      .select("id, driver_status, is_driver, role")
      .eq("id", driverId)
      .single();
