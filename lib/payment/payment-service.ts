...
export class PaymentService {
  ...
  constructor(supabase: SupabaseClient) {
    ...
    this.mtnMomoService = new MTNMomoService({
      ...
      targetEnvironment: (process.env.MOMO_TARGET_ENVIRONMENT || "sandbox") as
        | "sandbox"
        | "production",
    ...
    const orangeConfig = {
      ...
      environment: (process.env.ORANGE_MONEY_ENVIRONMENT || "sandbox") as
        | "sandbox"
        | "production",
      notificationUrl:
      ...
      returnUrl:
        process.env.ORANGE_MONEY_RETURN_URL ||
        "http://localhost:3000/payments/status",
    };

    // Use mock service if merchant credentials are not set
    if (
      !orangeConfig.merchantId ||
      !orangeConfig.merchantKey ||
      process.env.USE_MOCK_ORANGE_MONEY === "true"
    ) {
  ...
  async createPayment(request: PaymentRequest): Promise<PaymentResponse> {
    try {
      ...
      if (!booking.code_verified && booking.verification_code) {
        ...
        throw new Error(
          "Booking verification required before payment can be processed"
        );
      }
  ...
  private async handleMTNPayment(
    ...
    if (process.env.MOMO_TARGET_ENVIRONMENT === "sandbox") {
      console.log(
        " Using sandbox environment with test number:",
        formattedPhone
      );
      if (formattedPhone !== "237670000000") {
        console.warn(
          " Not a sandbox test number, defaulting to successful flow"
        );
  ...
  async checkPaymentStatus(
    ...
    try {
      ...
      if (provider === "mtn") {
        const momoStatus = await this.mtnMomoService.getPaymentStatus(
          transactionId
        );
        console.log(" MTN MOMO Status:", {
          transactionId,
  ...
  async handlePaymentCallback(
    ...
    console.log(
      `📝 Processing ${provider.toUpperCase()} payment callback:`,
      data
    );
    ...
    const paymentId =
      provider === "orange"
        ? data.externalId || data.transactionId
        : data.transactionId;
  ...
  private formatPhoneNumber(phoneNumber: string): string {
    ...
    return formattedPhone.startsWith("237")
      ? formattedPhone
      : `237${formattedPhone}`;
  }
      ...
      if (error) {
        ...
        const { data: manualReceipt, error: manualError } = await this.supabase
          ...
          .insert({
            ...
            receipt_number: `RECEIPT-${new Date().getFullYear()}-${Math.floor(
    ...
    try {
      ...
      if (newStatus === "completed") {
        ...
        if (verificationCode.error) {
          ...
          console.log(
            "✅ Generated verification code for booking:",
            payment.bookingId
          );
        }

        // Send push notifications about payment completion
        try {
              ...
              await fetch("/api/notifications/booking", {
                ...
                body: JSON.stringify({
                  notificationData: JSON.stringify({
                    ...
                    body: `Votre reservation pour ${ride.from_city} → ${ride.to_city} est confirmee. Le chauffeur va bientot verifier votre code.`,
                    data: {
                      bookingId: payment.bookingId,
              ...
              await fetch("/api/notifications/booking", {
                ...
                body: JSON.stringify({
                  notificationData: JSON.stringify({
                    ...
                    title: "💳 Paiement Recu !",
                    body: `Un passager a complete le paiement pour ${ride.from_city} → ${ride.to_city}. Verifiez le code de reservation.`,
                    data: {
                      bookingId: payment.bookingId,
                      rideId: booking.ride_id,
                      type: "payment_completed_driver",
          ...
          console.warn(
            "⚠️ Failed to send payment completion notifications:",
            notificationError
          );
        }
  ...
  private async generateVerificationCode(bookingId: string) {
    ...
    try {
      ...
      return await this.supabase.rpc("generate_booking_verification_code", {
