...
export function VehicleImagesUpload({
  ...
  const { t } = useLocale();
  const imageText = images && images.length > 1 ? t("pages.becomeDriver.vehicleImages.images") : t("pages.becomeDriver.vehicleImages.image");
        ...
        <div className="flex flex-col items-center justify-center h-32">
          {isLoading ? (
                  <div className="flex flex-col items-center">
                    ...
                    <p className="text-primary font-medium">{t("pages.becomeDriver.vehicleImages.uploading")}</p>
                    <p className="text-xs text-muted-foreground mt-1">{t("pages.becomeDriver.vehicleImages.uploadingDesc")}</p>
                  </div>
                ) : (
                  <div className="flex flex-col items-center text-center">
                    ...
                    <p className="font-medium text-gray-700 group-hover:text-gray-900">
                      {t("pages.becomeDriver.vehicleImages.clickToUpload")}
                    </p>
                    <p className="text-xs text-gray-500 mt-1 max-w-xs">
                      {t("pages.becomeDriver.vehicleImages.multipleImages")}
                    </p>
                  </div>
                )}
              </div>
      ...
      {images && images.length > 0 && (
        <div className="bg-white/80 rounded-lg p-4 shadow-sm">
          <div className="flex items-center justify-between mb-3">
            <h4 className="font-medium">{t("pages.becomeDriver.vehicleImages.uploaded")}</h4>
            <span className="text-xs text-muted-foreground">{images.length} {imageText}</span>
          </div>
            ...
            {images.map((url, index) => (
              <div key={index} className="relative group rounded-lg overflow-hidden aspect-square shadow-sm border border-gray-100">
                ...
                <div className="absolute inset-0 bg-gradient-to-t from-black/60 to-transparent opacity-0 group-hover:opacity-100 transition-opacity flex items-end p-2">
                  <span className="text-white text-xs font-medium">{t("pages.becomeDriver.vehicleImages.vehiclePhoto")} {index + 1}</span>
                </div>
              </div>
            ))}
          </div>
