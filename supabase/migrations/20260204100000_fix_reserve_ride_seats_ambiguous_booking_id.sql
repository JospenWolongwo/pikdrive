-- ============================================================================
-- Fix: Ambiguous column "booking_id" in reserve_ride_seats (UPDATE mode)
-- ============================================================================
-- Bug: When adding seats to an already-paid booking, the function raised:
--   column reference "booking_id" is ambiguous
--
-- Cause: The function RETURNS TABLE(..., "booking_id" uuid, ...). In PL/pgSQL
-- that output column name is in scope. The defensive validation query does
--   FROM payments WHERE booking_id = p_booking_id
-- so PostgreSQL cannot tell if "booking_id" means payments.booking_id or
-- the return column.
--
-- Fix: Qualify the column as payments.booking_id. No behavior change; only
-- disambiguation. No other edits to the function body.
-- ============================================================================
...
BEGIN
  ...
  IF p_booking_id IS NOT NULL THEN
    ...
    -- Calculate total paid vs total required (payments.booking_id to avoid ambiguity with RETURNS TABLE column)
    SELECT COALESCE(SUM(amount), 0)
    INTO v_total_paid
    FROM payments
    WHERE payments.booking_id = p_booking_id AND status = 'completed'::payment_status;
...
COMMENT ON FUNCTION "public"."reserve_ride_seats"(
  ...
  "p_seats" integer,
  "p_booking_id" "uuid"
) IS 'Creates or updates bookings with DEFENSIVE payment_status validation. Always verifies against actual payments. Fixed: ambiguous booking_id in defensive query (payments.booking_id).';
